  ...
  const stepMap: { [index: number]: { subtitle: string; form: JSX.Element } } =
    {
      0: {
        subtitle:
          "Insira o email de sua conta que enviaremos instruções para recuperá-la.",
        form: (
          <ForgotPasswordForm
            onSuccess={(token) => (setToken(token), nextStep())}
          />
        ),
      },
      1: {
        ...
        form: (
          <CheckTokenCodeForm
            tokenId={tokenId}
            onSuccess={(code) => (setTokenCode(code), nextStep())}
          />
        ),
      },
      2: {
      ...
      3: {
        ...
        form: (
          <>
            <Result status="success" title="Conta recuperada com sucesso!" />
