...
const StartActivity = () => {
  ...
  const loadActivity = async () => {
    ...
    form.setFieldsValue({
      answers: result.questions.map((v) => ({
        questionId: v._id,
        answer: null,
      })),
    })
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      await delay(2000)
      router.replace(
        `/v2/class/${router.query.classId}/activity/${router.query.activityId}`
      )
    } catch (err) {
        ...
        <Row gutter={[24, 24]}>
          ...
          <Col sm={24} xl={16}>
            <Form
              ...
              onInput={() => hasErrors && setHasErrors(false)}
            >
            ...
            <Card>
              {hasErrors && (
                <Typography.Text type="danger">
                  Ops! Falta responder algumas questões
                </Typography.Text>
              )}
              <Typography.Paragraph>
                Para entregar a atividade, clique no botão abaixo
              </Typography.Paragraph>
              <Button
                type="primary"
                onClick={() => form.submit()}
