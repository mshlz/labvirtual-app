                ...
                switch (node['name']) {
                    ...
                    case 'span':
                        if (node['attribs'] && node['attribs']['data-origin'] == 'glossary-entry-word') {
                            ...
                            current = <Typography.Text style={{ borderBottom: 'dashed 1px #333', cursor: 'pointer' }} onClick={() => openGlossaryEntry&&openGlossaryEntry(uuid)}>{text}</Typography.Text>
                        }
                        break;
                }
                return current
...
const SimulatorPage = () => {
    ...
    return !isLoading && <AdminLayout>
        <Space align="center" size={'middle'} style={{ marginBottom: '2rem' }}>
            ...
            <Typography.Title level={1} style={{ fontSize: '48px', marginBottom: 0 }}>{simulator.name}</Typography.Title>
        </Space>
