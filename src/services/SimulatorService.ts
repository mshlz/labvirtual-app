...
export class SimulatorService {
    ...
    static async getFromDisciplines(disciplines: string | string[]) {
        return (await Axios.post(`/simulators/from/disciplines`, { disciplines: Array.isArray(disciplines) ? disciplines : [disciplines] })).data.data
    }

    static async getFromSubjects(subjects: string | string[]) {
        return (await Axios.post(`/simulators/from/subjects`, { subjects: Array.isArray(subjects) ? subjects : [subjects] })).data.data
    }
}
