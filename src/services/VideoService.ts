...
export class VideoService {
    ...
    static async getFromDisciplines(disciplines: string | string[]) {
        return (await Axios.post(`/videos/from/disciplines`, { disciplines: Array.isArray(disciplines) ? disciplines : [disciplines] })).data.data
    }

    static async getFromSubjects(subjects: string | string[]) {
        return (await Axios.post(`/videos/from/subjects`, { subjects: Array.isArray(subjects) ? subjects : [subjects] })).data.data
    }
}
