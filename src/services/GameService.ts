...
export class GameService {
    ...
    static async getFromDisciplines(disciplines: string | string[]) {
        return (await Axios.post(`/games/from/disciplines`, { disciplines: Array.isArray(disciplines) ? disciplines : [disciplines] })).data.data
    }

    static async getFromSubjects(subjects: string | string[]) {
        return (await Axios.post(`/games/from/subjects`, { subjects: Array.isArray(subjects) ? subjects : [subjects] })).data.data
    }
}
