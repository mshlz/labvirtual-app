...
export class LessonService {
    ...
    static async getFromDisciplines(disciplines: string | string[]) {
        return (await Axios.post(`/lessons/from/disciplines`, { disciplines: Array.isArray(disciplines) ? disciplines : [disciplines] })).data.data
    }

    static async getFromSubjects(subjects: string | string[]) {
        return (await Axios.post(`/lessons/from/subjects`, { subjects: Array.isArray(subjects) ? subjects : [subjects] })).data.data
    }
}
