...
// Admin
const adminTranslationMap: { [key: string]: (router: NextRouter) => IBreadcrumbItem } = {
    'manager': router => ({
        name: 'Administração'
    }),
    'create': router => ({
...
const translationMap: { [key: string]: (router: NextRouter) => IBreadcrumbItem } = {
    'class': router => ({
        name: 'Minhas Turmas',
        href: '/v2/my-classes'
    }),
...
const skipParts = (value) => !(['', 'v2'].includes(value) /*|| /^\[.+\]$/.test(value)*/)
...
export const AutoBreadcrumb = () => {
    ...
    useEffect(() => {
        ...
        const items = router.pathname.split('/')
            .filter(skipParts)
            .map(i => ({
                ...(isAdmin ? adminTranslationMap[i] : translationMap[i])?.call(this, router),
                key: i
            }))
    ...
    const generateParts = (items: IBreadcrumbItem[]) => {
        return items.map(item => <Breadcrumb.Item key={item.key || item.name}>
    ...
    return (
        <Breadcrumb style={{ margin: '24px', marginBottom: 0 }}>
            {breadcrumbs.map(item => <Breadcrumb.Item key={item.key || item.name}>
                {item.href ? <Link href={item.href}>{item.name}</Link> : item.name}
            </Breadcrumb.Item>)}
        </Breadcrumb>
    )
