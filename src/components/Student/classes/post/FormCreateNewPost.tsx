...
export const FormCreateNewPost = ({
    ...
    const isValid = await ValidateForm(
      {
        text: Yup.string()
          .trim()
          .matches(/<[^>]+>[^<]+/gm, "você precisa escrever alguma coisa")
          .max(5000)
          .required(),
    ...
    <div className="card border-0 shadow-sm">
      <div className="card-body rounded text-white">
        ...
        <Form onSubmit={handleSubmitPost} ref={formRef}>
          <RichTextEditor
            name="text"
            buttons={[["bold", "underline", "italic", "list", "removeFormat"]]}
          />
          <Button color="primary" outline cssClasses="float-right">
            Postar
          </Button>
        </Form>
      </div>
