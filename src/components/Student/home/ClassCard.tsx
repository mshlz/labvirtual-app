...
export const ClassCard = ({ id, name, teacher }: IClassCard) => {
  return (
    <div
      className="col-xl-4 col-lg-6 col-md-6 col-sm-6 mb-3"
      style={{ maxWidth: "450px" }}
    >
      <div className="card border-0 shadow-small m-b-10">
        <div
          className="card-header pt-3 align-items-center"
          style={{ backgroundColor: hashToColor(`${id + name + teacher}`) }}
        >
          {/* '#8bc34a' */}
          <div>
            <Link href={`/c/${id}`}>
              <h3 style={{ color: "#fff", fontWeight: 400, cursor: "pointer" }}>
                {name}
              </h3>
            </Link>
