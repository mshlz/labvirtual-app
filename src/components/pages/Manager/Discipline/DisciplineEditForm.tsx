...
export const DisciplineEditForm = ({ disciplineId }: IDisciplineFormProps) => {
  ...
  const handleSubmit = async (data) => {
    try {
      ...
      if (disciplineId) await DisciplineService.update(disciplineId, data)
      else await DisciplineService.create(data)

      toast(
        `Disciplina ${disciplineId ? "atualizada" : "criada"} com sucesso!`,
        { type: "success" }
      )
    } catch (err) {
