...
export const SimulatorEditForm = ({ simulatorId: id }: ISimulatorFormProps) => {
  ...
  const loadSimulator = async () => {
    ...
    if (
      Array.isArray(simulator.disciplines) &&
      typeof simulator.disciplines[0] !== "string"
    ) {
  ...
  const handleSubmit = async (data) => {
    try {
      ...
      if (simulatorId) await SimulatorService.update(simulatorId, data)
      else {
        const result = await SimulatorService.create(data)
        setSimulatorId(result._id)
      }

      toast(`Simulador ${simulatorId ? "atualizado" : "criado"} com sucesso!`, {
        type: "success",
      })
    } catch (err) {
