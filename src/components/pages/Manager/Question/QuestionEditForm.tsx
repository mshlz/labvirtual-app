...
export const QuestionEditForm = ({ questionId: id }: IQuestionFormProps) => {
  ...
  const handleSubmit = async (data) => {
    try {
      ...
      if (questionId) await QuestionService.update(questionId, data)
      else {
        const result = await QuestionService.create(data)
        setQuestionId(result._id)
      }

      toast(`Questão ${questionId ? "atualizada" : "criada"} com sucesso!`, {
        type: "success",
      })
    } catch (err) {
              ...
              {subjects.map((v) => (
                <Select.Option key={v._id} value={v._id}>
                  ...
                  <Typography.Text type="secondary">
                    -{" "}
                    {disciplines.find((d) => d._id === v.discipline)?.name ||
                      ""}
                  </Typography.Text>
                </Select.Option>
              ))}
            ...
            >
              <Radio.Group
                options={[
                  ...
                  {
                    label: "Múltipla escolha (multi resposta)",
              ...
              <div className="form-group">
                      ...
                      validator: async (_, alternatives) => {
                        if (!alternatives || alternatives.length < 2) {
                          return Promise.reject(
                            new Error(
                              "Você precisa adicionar ao menos 2 alternativas"
                            )
                          )
                        }
                      },
                ...
                >
                  {(fields, { add, remove }, { errors }) => (
                    <>
                      {fields.length === 0 && (
                        <Empty description="Não há alternativas" />
                      )}
