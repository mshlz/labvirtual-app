...
export const GlossaryEditForm = ({ glossaryItemId }: IGlossaryFormProps) => {
    const [form] = useForm()
    ...
    const loadGlossaryEntry = async () => {
        ...
        await loadSubjects(glossaryEntry.discipline?._id || glossaryEntry.discipline)
    ...
    const handleSubmit = async (data) => {
        try {
            ...
            toast(`Conteúdo ${glossaryItemId ? 'atualizada' : 'criada'} com sucesso!`, { type: 'success' })
        } catch (err) {
    ...
    return <>
        <PageHeader
            title={glossaryItemId ? "Editar item do glossário" : "Criar nova item para o glossário"}
        ...
        <Card title="Informações básicas" loading={isLoading}>
            <Form name="basic" form={form} layout="vertical" onFinish={handleSubmit} onValuesChange={handleValuesChange}>
                <Form.Item label="Nome do termo" name="name" rules={[{ required: true, min: 3 }]}>
                    <Input />
                </Form.Item>
