...
export const PageEditForm = ({ pageId: id }: IPageFormProps) => {
    ...
    const handleSubmit = async (data) => {
        try {
            ...
            toast(`Página ${pageId ? 'atualizada' : 'criada'} com sucesso!`, { type: 'success' })
        } catch (err) {
    ...
    return <>
        ...
        <Card title="Informações básicas" loading={isLoading}>
            <Form name="basic" form={form} layout="vertical" onFinish={handleSubmit}>
                <Form.Item label="Autor" name="authorName" rules={[{ required: true, min: 3 }]}>
                    <Input />
                </Form.Item>
                <Form.Item label="Nome da página" name="name" rules={[{ required: true, min: 3 }]}>
                    <Input />
                </Form.Item>
                <Form.Item label="Seção" name="section" rules={[{ required: true }]}>
                    ...
                    >
                        {sections.map(v => <Select.Option value={v._id}>{v.name}</Select.Option>)}
                    </Select>
                </Form.Item>
