...
export const VideoEditForm = ({ videoId: id }: IVideoFormProps) => {
  ...
  const loadVideo = async () => {
    ...
    if (
      Array.isArray(video.disciplines) &&
      typeof video.disciplines[0] !== "string"
    ) {
  ...
  const handleSubmit = async (data) => {
    try {
      ...
      if (videoId) await VideoService.update(videoId, data)
      else {
        const result = await VideoService.create(data)
        setVideoId(result._id)
      }

      toast(`Video ${videoId ? "atualizado" : "criado"} com sucesso!`, {
        type: "success",
      })
    } catch (err) {
