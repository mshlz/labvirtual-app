...
export const InstitutionEditForm = ({
  institutionId,
}: IInsitutionFormProps) => {
  const [form] = useForm()
  ...
  const handleSubmit = async (data) => {
    try {
      ...
      if (institutionId) await InstitutionService.update(institutionId, data)
      else await InstitutionService.create(data)

      toast(
        `Instituição ${institutionId ? "atualizada" : "criada"} com sucesso!`,
        { type: "success" }
      )
    } catch (err) {
