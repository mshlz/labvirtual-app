  ...
  return (
    <Form form={form} onFinish={handleOnFinish}>
      <Form.Item
        name="text"
        rules={[
          { required: true, message: "Você precisa escrever alguma coisa" },
        ]}
        extra={
          <Button
            htmlType="submit"
...
const MuralPost = (props: IMuralPost) => {
      ...
      .then((result) => {
        ...
        setComments([
          ...comments,
          {
            ...
            content: result.text,
            datetime: (
              <Tooltip title={<p>{result.createdAt}</p>}>
  ...
  const handleRemoveComment = async (id: string) => {
    await CommentService.delete(id).then((result) => {
  ...
  return (
    ...
    >
      <Card.Meta
        ...
        description={props.date && relativeDate(props.date)}
      />

      {props.content && (
        <Typography.Paragraph style={{ marginTop: 16 }}>
          <div className="post-content">{parseHtml(props.content)}</div>
        </Typography.Paragraph>
      )}

      {comments?.length > 0 && (
        <List
          header={`${comments.length} comentário${
            comments.length > 1 ? "s" : ""
          }`}
              ...
              <Comment
                actions={
                  user._id === item.author._id
                    ? [
                        <Popconfirm
                          title="Você tem certeza？Essa ação é irreversível"
                          okText="Sim"
                          cancelText="Não"
