...
export const ActivityEditForm = ({
  ...
  const handleSubmit = async (data) => {
    ...
    const reqData = {
      ...
      questions: (data.questions || [])
        .filter((v) => !v._new)
        .map((v) => ({ questionId: v._id, value: v.value })),
    ...
    try {
      ...
      message.success(
        `Atividade ${classworkId ? "atualizada" : "criada"} com sucesso!`
      )
      await delay(2000)
  ...
  const handleSearch = async (query: string) => {
    ...
    setIsSearching(true)
    const skipIds = newQuestions
      .map((v) => v._id)
      .concat(questions.map((v) => v.parent))
  ...
  const renderQuestion = (item: {
    ...
    name: string
    text?: string
    alternatives?: { correct: boolean; text: string; code: string }[]
  }) => {
    return (
      item && (
        <>
          ...
          {item.alternatives && (
            <Radio.Group
              onChange={() => {}}
              size="large"
              value={item.alternatives.find((v) => v.correct)?.code}
            >
              <Space direction="vertical">
                {item.alternatives.map(
                  (v, i) => (
                    <Radio key={i} value={v.code} checked={v.correct}>
                      {parseHtml(v.text || "")}
                    </Radio>
                  )
  ...
  const renderSearchQuestion = () => {
    return (
      <Card
        loading={isFetching}
        title={
          <Typography.Title level={4}>Adicionar Questões</Typography.Title>
        }
      >
        <Input.Search
        ...
        {searchResult && searchResult.length ? (
          <Typography.Title level={5} style={{ marginTop: "16px" }}>
            {searchResult.length} resultado
            {searchResult.length !== 1 ? "s" : ""} para a pesquisa:
          </Typography.Title>
        ) : (
          <></>
        )}
        ...
        <List
          ...
          renderItem={(item) => (
            <List.Item
              actions={[
                <Button onClick={() => openPreviewQuestion(item._id)}>
                  Preview
                </Button>,
                <Button onClick={() => addNewQuestion(item)} type="primary">
                  Clone
                </Button>,
              ]}
            >
              <List.Item.Meta
                title={
                  <a target="_blank" href={`/manager/questions/${item._id}`}>
                    {item.name}
                  </a>
                }
        ...
        <Modal
          ...
          footer={
            <Button
              ...
              onClick={() =>
                previewData &&
                (addNewQuestion(previewData), setPreviewOpen(false))
              }
            >
              Adicionar
            </Button>
        ...
        >
          <Result
            style={{ display: !isPreviewLoading ? "none" : "initial" }}
            icon={<Spin />}
          />
      ...
      <Row gutter={[24, 24]}>
        <Col span={24}>
          <Typography.Title level={2}>
            {classworkId ? "Editar" : "Nova"} atividade
          </Typography.Title>
        </Col>
        <Col span={24}>
          <Form
            ...
            <Row gutter={[24, 24]}>
              <Col span={18}>
                <Row gutter={[24, 24]}>
                  ...
                  <Col span={24}>{renderSearchQuestion()}</Col>
                      ...
                      <>
                        ...
                        {fields.map((item, idx) => (
                          <Col span={24} key={item.key}>
                            ...
                            >
                              {renderQuestion(
                                form.getFieldValue(["questions", idx])
                              )}
                            </Card>
                          </Col>
                        ))}
