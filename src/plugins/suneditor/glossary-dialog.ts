...
export const GlossaryEntryPlugin: Plugin = {
    ...
    add: function (core) {
        ...
        context.glossaryEntry.focusElement = glossary_dialog.querySelector('._se_link_url');
        context.glossaryEntry.targetSelect = glossary_dialog.querySelector('.se-input-select');
        context.glossaryEntry.formBody = glossary_dialog.querySelector('#formBody');

        glossary_dialog.querySelector('form').onsubmit = event => this.fetchGlossaryItems.call(this, core, event); // ? i think i dont need anymore
        glossary_dialog.querySelector('#searchBtn').onclick = event => this.fetchGlossaryItems.call(this, core, event); // event listener to search button
    ...
    open: function () {
        ...
        this.plugins.dialog.open.call(this, 'glossaryEntry', 'glossaryEntry' === this.currentControllerName);
    },
    ...
    fetchGlossaryItems: function (core, event) {
        ...
        GlossaryService.simpleSearch({ query })
            .then(data => {
                ...
                core.context.glossaryEntry.formBody.appendChild(this.generateListComponent(core, data));
            })
            .catch(err => {
                core.context.glossaryEntry.formBody.innerHTML = this.renderNoResult(err.message || "Ops! Ocorreu um erro.");
            })
    },
    ...
    renderNoResult: function (msg) {
        return `<div class="align-items-center d-flex flex-column justify-content-center mt-3">
                    <i class="fa fa-2x fa-warning mb-1 text-muted"></i><small>${msg || 'Nenhum resultado encontrado'}</small>
                </div>`
    },
    ...
    insertEntry: function (core, entry: { id: string, name: string }) {
        core.functions.insertHTML(`<span title="glossário" data-origin="glossary-entry-word" id="${entry.id}">${entry.name}</span>`, true);
